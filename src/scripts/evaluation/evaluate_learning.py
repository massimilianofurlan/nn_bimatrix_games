...
_, _, model_metadata, model_dir = select_models(model_dir = args.model)
...
exp = np.ceil(np.log10(model_metadata['optimization_steps'])).astype(int)
model_log_steps = np.unique(np.logspace(0, exp, num=20*exp+1, dtype=int))
model_log_steps = model_log_steps[model_log_steps <= model_metadata['optimization_steps']]
evaluation_outputs = []
for step in model_log_steps:
    # Load models
    model1 = load_model(model_metadata, f'models/{model_dir}/models_log/model1_{step}.pth', device).eval()
    model2 = load_model(model_metadata, f'models/{model_dir}/models_log/model2_{step}.pth', device).eval()
...
model_log_steps = np.append(model_log_steps,model_metadata['optimization_steps'])
# SAVE WHEN COMPUTING
...
n_actions = model_metadata['n_actions']
